 ...
 * strings se numbers parse karna. Tum Bhaiyya ji ka meter software banao!
 *
...
export function findCheapestAndCostliest(...fares) {
  ...
  *      - Example: findCheapestAndCostliest(150, 80, 200) => { cheapest: 80, costliest: 200 }*/
  ...
  let vfare = fares.filter((fare) => Number.isFinite(fare))
  if(vfare.length === 0){
    return null
  }

  return {cheapest: Math.min(...vfare), costliest: Math.max(...vfare)}
 ...
 */
  ...
  let fareBhai = Math.abs(parseInt(to)-parseInt(from))

  if(Number.isFinite(fareBhai)){
    return fareBhai
  }
  return -1
