 ...
 */
 ...
 const vegCount = thalis.filter(i=>i.isVeg).length
 const nonVegCount = thalis.filter(i=>!i.isVeg).length

 const totalPrice = thalis.reduce((sum,i)=> sum + i.price,0)
 ...
 */
   ...
   return thalis.filter(thali => {
    const nameKarn = thali.name.toLowerCase().includes(queries)
    const itemMatch = thali.items.some(item =>
      item.toLowerCase().includes(queries)
    )
    return nameKarn || itemMatch;
   })
